...
SAVE_DIR=./perturb_results/raw_softmax
SEED=2021
LR=0.0001
...
for noise in "${NOISES[@]}"; do
  for atten in "${ATTENS[@]}"; do
    CUDA_VISIBLE_DEVICES=${GPU} ${PYTHON_BIN} -u run.py \
      ...
      --data_path ETTh1.csv \
      --model_id ETTh1_336_96_raw_softmax_noise${noise}_atten${atten}_seed${SEED} \
      --model ${MODEL_NAME} \
      --data ETTh1 \
      --features M \
      --seq_len 336 \
